...
$targetBase = blu_robot_pieseauto_effective_url();
$path = $_GET['path'] ?? '/';
...
$ch = curl_init($url);
$quickPath = in_array($path, ['/verificare_sesiune', '/get_status', '/este_ocupat'], true)
...
curl_setopt_array($ch, [
    ...
    CURLOPT_TIMEOUT => $quickPath ? 10 : 180,
...
if ($response === false) {
    ...
    echo json_encode([
        'status' => 'error',
        'mesaj' => 'Robot PieseAuto indisponibil. Pornește robot_pieseauto.py (verifică ROBOT_PIESEAUTO_URL din .env).',
        'detalii' => $error,
        'target' => $targetBase,
